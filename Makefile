...
LD_FLAGS := -s -w \
    -X github.com/cosmos/cosmos-sdk/version.Name=thari \
    -X github.com/cosmos/cosmos-sdk/version.AppName=tharid \
...
install: mod-vendor
ifeq (${OS},Windows_NT)
	go build -mod=readonly ${BUILD_FLAGS} -o ${GOBIN}/tharid.exe ./cmd
else
	go build -mod=readonly ${BUILD_FLAGS} -o ${GOBIN}/tharid ./cmd
endif

build:
ifeq (${OS},Windows_NT)
	go build  ${BUILD_FLAGS} -o ${GOBIN}/tharid.exe ./cmd
else
	go build  ${BUILD_FLAGS} -o ${GOBIN}/tharid ./cmd
endif

release: build
	mkdir -p release
ifeq (${OS},Windows_NT)
	tar -czvf release/tharid-${GOOS}-${GOARCH}.tar.gz --directory=build/${GOOS}/${GOARCH} tharid.exe
else
	tar -czvf release/tharid-${GOOS}-${GOARCH}.tar.gz --directory=build/${GOOS}/${GOARCH} tharid
endif
